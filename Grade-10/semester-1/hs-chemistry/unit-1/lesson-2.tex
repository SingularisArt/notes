\lesson{2}{Sep 09 2021 Thu (08:20:00)}{Measuring Matter}

All measurements are made of \textbf{2} parts:
\begin{itemize}
	\item A number.
	\item A unit.
\end{itemize}
...
\begin{definition}[Matter]
	Anything that takes up space, whether big or small, is \textbf{Matter}.
\end{definition}

\begin{definition}[Volume]
	In science, the amount of space an object occupies is called \textbf{volume}
\end{definition}

\begin{definition}[Mass]
	\textbf{Mass} is a measure of the amount of matter in an object. This means, the more \textbf{Matter} an object has, the object's \textbf{Mass} increases. \\
	Scientists measure the \textbf{Mass} of an object using the base \textbf{Unit Gram}.
\end{definition}

\subsubsection*{U.S. Customary System}

The \textbf{U.S. Customary System (English System)} consists of measurements we use every day to describe:
\begin{itemize}
...
The \textbf{United States} is the one of three countries in the \textbf{whole world} that use this system.
...
\begin{center}
	\begin{table}[h]
		...
		\caption{The \textbf{U.S. Customary System (English System)}}
...
The \textbf{Metric System} is a system of measuring units based on the power of
...
One reason for the widespread use of the \textbf{Metric System} is that it
...
for length, the \textbf{Metric System} uses only one word for the length, the
\textbf{Meter}.
...
inch. A prefix works with any unit of measure in the \textbf{Metric System}:
...
\begin{center}
	\begin{table}[h]
		...
		\caption{The \textbf{Metric System Prefix References}}
		\label{table:metric-system}
	\end{table}
\end{center}

\subsubsection*{The International System of Measurement}

The \textbf{International System of Measurement of Units} is the primary system
used by scientists and engineers. It will also be used in \textbf{Chemistry}.
...
\begin{definition}[Dimensional Analysis] Analyzing the relationships between
	...
	\textbf{Dimensional Analysis} uses a ratio, or a conversion factor to change
	units of measurement while maintaining the value of measurement.

	In a \textbf{Conversion Factor}, the \textbf{numerator} and \textbf{denominator} equal the same amount, just in different units.
	...
	\[
	1 = \frac{1 \textrm{ mile}}{5,280 \textrm{ feet}} \textrm{ or } 1 = \frac{5,280 \textrm{ feet}}{1 \textrm{ mile}}
	.\]
...
\begin{definition}[Derived Units]
	Some measurements you are familiar with, such as speed, contain more than one \textbf{Unit}.

	A very common \textbf{Derived Unit} is \textbf{Volume}. For liquids and gasses, \textbf{Volume} tends to be measured in liters. But, for regular-shaped solids, measurements and calculations can determine their \textbf{Volume}. To calculate the \textbf{Volume} of the cube in Figure~\ref{fig:3-3-3-cube}, just raise the side measurement to the third power.

	\begin{align*}
		V = (3 cm)^3 \\
		V = (3 \times 3 \times 3) cm \\
		V = 27 cm^3 
	\end{align*}
\end{definition}

Now, let's learn how to convert \textbf{Derived Units}:
...
\begin{example}[Fractional Unit]
	...
	factional unit, there may be more steps to the \textbf{Dimensional Analysis}.
	...
	\[
		\frac{0.36 \textrm{ \sout{km}}}{1 h} \times \frac{1,000 \textrm{ \sout{m}}}{1 \textrm{ \sout{km}}} \times \frac{100 cm}{1 \textrm{ \sout{m}}}
	.\]
	...
	\[
		\frac{36,000 cm}{1 \textrm{ \sout{h}}} \times \frac{1 \textrm{ \sout{h}}}{60 \textrm{ \sout{min}}} \times \frac{1 \textrm{ \sout{min}}}{60 sec}
	.\] 
	...
	the minutes unit is canceld to leave seconds in the denominator -- where it
	needs to be for the final answer:

	\begin{align*}
		\frac{36,000 cm}{1 \textrm{ \sout{h}}} \times \frac{1 \textrm{ \sout{h}}}{60 \textrm{ \sout{min}}} \times \frac{1 \textrm{ \sout{min}}}{60 sec} = 10 \frac{cm}{s} \\
		0.36 \frac{km}{h} = 10 \frac{cm}{s}
	\end{align*}
\end{example}

\begin{example}[Powered Unit] Some units are raised to a power.
	\textbf{Area} and \textbf{Volume} of solids are the common ones.

	Area is measured in squared length, such as the square meter, $m^2$, while
	\textbf{Volume} is measured in cubic length, such as the cubic meter, $m^3$.
	...
	Convert the \textbf{Volume} $5.0 m^3$ to the unit $cm^3$.
	...
	This makes the fraction a three dimensional \textbf{Volume} measurement
	...
	The units $m^3$ and $m$ cannot cancel out because they are not exactly the same. The entire conversion factor must be cubed in order to make the units match:

	\begin{align*}
		5.0 m^3 \times \frac{100 cm}{1 m} \\
		5.0 m^3 \times \left( \frac{100 cm}{1 m} \right)^3 \\
		5.0 m^3 \times \left( \frac{1,000,000 cm^3}{1 m^3} \right)
	\end{align*}

	Now that the cubic meter units are able to cancel, it's time to calculate the answer. Because the entire fraction is being cubed, the numbers and units must all be cubed:

	\begin{align*}
		5.0 m^3 \times \frac{1,000,000 cm^3}{1 m^3} \times \frac{5.0 \times 1,000,000 cm^3}{1} \\
		=5,000,000 cm^3 \textrm{ or } 5 \times 10^6 cm^3
	\end{align*}
